...
mortalidade_ufs <- dados_sih %>%
  ...
  mutate(n_cem_mil = round((n_obitos / n_int) * 100000, 1))
...
mapa_mort <- ggplot(data = mapa_dados) +
  geom_sf(color = "grey50", size = 0.1, aes(fill = n_cem_mil, geometry = geom)) +
  scale_fill_gradient(
    low = "lavender",
    high = "mediumpurple",
    na.value = "white"
...
ggsave("mapa_mort_mat.png", plot = mapa_mort, width = 10, height = 6, dpi = 800)
